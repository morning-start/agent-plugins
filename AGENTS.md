...
- **任务验收后的 Git 提交契约**：单个任务 → 交给用户确认，不自动提交；多个任务且用户已授权提交、且项目为 git 仓库 → 验收后按授权提交（单次提交只含一个 Task 产物，遵循 Conventional Commits）。非 git 仓库只展示变更，不执行 git 命令。详见 `moonbit-implement` 的「任务验收后的 Git 提交契约」。
- **多任务批次上限**：连续执行任务每批最多 5 个，之后停止汇报并进入提交检查点（提交或交用户确认后压缩会话/开始新上下文再继续）；平台不支持会话压缩时批次上限收紧为 ≤3。批次边界由 `moonbit-writing-plans` 规划、`moonbit-implement` 执行、`moonbit-git` 提交检查点衔接。
