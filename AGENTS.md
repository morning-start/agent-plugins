...
- **任务验收后的 Git 提交契约**：项目为 git 仓库时，每个任务验收通过后**默认自动执行**「建功能分支 → 提交（单次提交只含一个 Task 产物，遵循 Conventional Commits）→ 合并回主分支 → 删除分支」，无需逐任务等待授权；用户明确要求不自动提交/不合并时例外。非 git 仓库只展示变更，不执行 git 命令。详见 `moonbit-git` 的「提交契约」。
