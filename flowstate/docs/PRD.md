...
- **图编排（Agent Graph）**：全流程是**一张可执行的状态图**（见第七章）——节点=流程环节、边=DoD 流转条件、条件边=变更分级路由、循环=变更回环与迭代闭环、人工闸门（HITL）=必须等人确认的节点、Checkpoint=断点续跑。运行时参考 LangGraph 等图编排模型
...
#### F4.1 开发执行指引（docs/plan → docs/task）

> 铁律：**不写 plan 不写码，不分批不动工**。开发动作开始前，必须先产出 plan 与 task。
>
> 存放约定：过程态产物（plan/task/实验脚本）统一放 **`.agent-workplace/`**（不提交 git）；
> 要提交的内容从一开始就写在项目正式目录。详见 `docs/agent-workplace.md`。

**第一步：写 `docs/plan`（计划，按大阶段分 phase）**

- 根据本次迭代范围，按**大阶段**拆分多个 phase，例如：基础层 → 核心流程 → 交互/外围 → 打磨/上线准备
- 每个 phase 必须写清楚两件事：
  - **要做什么**（本 phase 的目标与交付内容，关联需求 id）
  - **为什么做**（业务/技术理由，防止"为做而做"）
- phase 之间体现依赖顺序：前一个 phase 是后一个的基础

**第二步：细化 `docs/task`（任务，分批次）**
...
> 本插件的流程不是一份线性文档，而是一张**可执行的状态图（State Graph）**。AI 助手按图遍历执行：节点是流程环节，边是流转条件，循环/分支/人工闸门显式建模。运行时参考 LangGraph 等图编排模型（节点、条件边、循环、Checkpoint、HITL）。
...
5. **编排运行时选型（图编排落地）**：决定第七章执行图的承载方式——(a) 自研轻量状态机（纯 skills 驱动，适合 Claude Code / pi 的 skills-only 模型）；(b) 对接 LangGraph 等外部编排框架（支持 Checkpoint/HITL/并行，但引入运行时依赖）。默认建议 (a)，图为"逻辑蓝图"，由 skills 逐节点实现，Checkpoint 落在产出物文件上
...
- 内化了 **Agent 图编排（Graph Engineering）**：流程从线性文档升级为**可执行状态图**（节点/边/条件边/循环/HITL/Checkpoint/并行/紧急短路），供运行时按图驱动 AI 助手执行
