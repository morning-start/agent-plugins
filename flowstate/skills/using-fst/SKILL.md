...
| 场景 | 技能 | 管哪些节点 |
|------|------|-----------|
| 新项目启动、需求模糊、"做个 X" | `fst-init`（新项目路径） | N1 立项 / N2 冻结 / N3 设计 |
| 已有项目接入 flowstate、"帮我把这个项目管起来" | `fst-init`（已有项目路径） | N1 立项 / N2 冻结 / N3 设计 |
...
- 新项目/需求模糊 → `fst-init`（新项目路径）
- 已有项目接入 flowstate → `fst-init`（已有项目路径：探索现状 → 评估 → 迁移 → 锁定底线）
- 迭代中任何变更/缺陷修复 → `fst-change`
- 验收/上线前 → `fst-review`
- 开发/回顾 → `fst-iterate`
- 已批准范围内的一句话 diff → 进入 `fst-iterate`，走 lightweight todo 并做最小验证；如果是新需求或缺陷，先走 `fst-change`
