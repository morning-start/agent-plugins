...
节点/边/条件边/循环/人工闸门/检查点的**权威定义与完整 Mermaid 见
[`references/flow-graph.md`](references/flow-graph.md)**；此处只给读图要点，不复述细节表。

- **DoD 守卫**：未核销不得沿边前进（如 N1 → N2 需 3 条底线书面确认、N3 → N4 需 PRD 评审通过）
- **三条回环**：变更回环（N4→N5→N4，重大→N3 暂停重启）、迭代闭环（N8→N4）、Hotfix 短路（任意节点→N9→N6）
- **HITL**：底线确认 / 范围签署 / PRD 评审 / 变更分级与重大审批 / DoD 核销 / 放量全量决策——**强制暂停等人**
- **Checkpoint**：每节点完成即落 `.agent-workplace/state/checkpoint.json`，中断可断点续跑
