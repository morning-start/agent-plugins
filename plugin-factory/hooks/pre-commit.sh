#!/usr/bin/env bash
# plugin-factory pre-commit gate (bash variant).
...
if printf '%s\n' "$staged" | grep -q '^\.agent-workplace/'; then
  echo "[plugin-factory] BLOCKED: .agent-workplace/ is the agent's PRIVATE workspace and must never be committed." >&2
  echo "[plugin-factory] Remove it from the index: git reset .agent-workplace/ (content stays on disk)." >&2
  exit 1
fi
...
if [ -n "$secret_hits" ]; then
  echo "[plugin-factory] BLOCKED: possible secret in staged changes — check before committing." >&2
