 ...
 * in sync with scripts/routing-table.json (the single source of truth).
 * Only runs when scripts/routing-table.json exists — generated plugins do not
 * ship it, so the check is a no-op for them. Emits a FAIL finding on drift.
 */
async function routingDriftCheck(root, findings) {
  const tablePath = join(root, "scripts", "routing-table.json");
  try {
  ...
  const { renderRoutingTables } = await import(
    pathToFileURL(join(root, "scripts", "routing-render.mjs")).href
  );
  ...
  for (const [name, expected] of markers) {
    ...
    const expectedBlock = `<!-- ${name}:START (generated by scripts/render-routing.mjs from scripts/routing-table.json — do not edit by hand) -->\n${expected}\n<!-- ${name}:END -->`;
    if (!match || match[0] !== expectedBlock) {
      findings.push(
        makeFinding(
          ...
          "FAIL",
          "Run node scripts/render-routing.mjs to re-render the routing tables from scripts/routing-table.json.",
          `The ${name} table drifted from the single source of truth (scripts/routing-table.json).`,
        ),
      );
    }
  }
