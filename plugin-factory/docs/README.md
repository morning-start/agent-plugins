...
| **Reference（参考）** | 查规范：标准、契约、约定、探针、适配器 | `references/`（设计原则、编排模式、生命周期矩阵、适配器/插件规格）· ADR 约定见 `skills/pf-adr/SKILL.md` · `docs/glossary.md`（术语）· `schemas/`（交接产物 JSON Schema 契约）· `scripts/`（可执行门禁：verify/bundle/package）· `roles/`（子代理提示词） |
