# tools/scaffold — 插件生成器

跨平台多 harness 插件 scaffold 渲染器：把 `templates/` 与每个请求的 harness
模板（`templates/harnesses/<h>/`）渲染进新目标目录。`scaffold.sh` / `scaffold.ps1`
是薄参数包装，实现只有 `scaffold.mjs`。

```
scaffold.mjs   单一实现（函数式模板替换，非 shell 语法）
scaffold.sh     bash 包装
scaffold.ps1    PowerShell 包装
```

## 用法

```sh
node tools/scaffold/scaffold.mjs --name <plugin> --prefix <pfx> --target <dir> \
  [--description "..."] [--user-lang zh-CN] [--harnesses a,b,c] [--auto-verify]
```

支持 harness：`claude-code`、`pi`、`opencode`、`oh-my-pi`、`codex`。

## 插件模型（本模块的产出契约）

一个抽象模型描述所有 agent 插件；各端适配器把它映射为具体位置与格式。
技能按 Agent Skills 标准编写一次、按端渲染——逐端映射见
`references/README.md`。

**职责边界（铁律 5）**：一个插件 = **一个固定业务场景**下的一整套任务；多场景
聚合是反例。判定见 `references/design-principles.md` 的 6 维发散检查（D1–D6）。
...
8. 若插件使用自定义验证器（非 `scripts/verify.mjs`），须在 `AGENTS.md` 的
...
### 产物不变量（scaffold 契约，`scaffold.mjs` 强制）
