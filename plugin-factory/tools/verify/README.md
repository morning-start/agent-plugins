...
| 信号（纯结构） | 探针（`verify.mjs lifecycle`） | 严重度 | 如何度量 | 建议 |
|--------------------------|----------------|---------|----------------|----------------|
...
| 评测通过率 | `eval-pass-rate` | `tools/design/evals.json`（每个技能的评测用例 + 通过率） | 通过率持续 < 阈值（如 70%）的技能升级为 **重写/重组**（走 skill-creator TDD 循环）；回归信号阻断发布 |
...
2. **v2.1** — `eval-pass-rate`：读取 `tools/design/evals.json`，与 `pf-verify` 的
   测试覆盖检查联动（见 pf-verify VFY-2）。
