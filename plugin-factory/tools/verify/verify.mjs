...
async function repeatedGuidanceCheck(root, skills, findings) {
  ...
  try {
    ...
    for (const line of ss.split(/\r?\n/)) {
      const m = /^##\s+(.+)$/.exec(line);
      if (m) rootSkeleton.add(m[1].trim().toLowerCase());
    }
  } catch {
    /* no skeleton declaration — nothing to add */
  }
  ...
  for (const s of skills) {
    ...
    for (const line of s.text.split(/\r?\n/)) {
      ...
      if (m) {
        ...
        if (SKELETON_HEADINGS.has(key) || rootSkeleton.has(key)) continue;
...
/* ------------------------------------------------------------------ */
/* Routing-table drift (plugin-factory itself only)                    */
/* ------------------------------------------------------------------ */

/**
 * plugin-factory's own routing tables in skills/using-pf/SKILL.md must stay
 * in sync with tools/routing/routing-table.json (the single source of truth).
 * Only runs when tools/routing/routing-table.json exists — generated plugins do
 * not ship it, so the check is a no-op for them. Emits a FAIL finding on drift.
 */
async function routingDriftCheck(root, findings) {
  const tablePath = join(root, "tools", "routing", "routing-table.json");
  try {
    await stat(tablePath);
  } catch {
    return; // not plugin-factory itself
  }
  ...
  const { renderRoutingTables } = await import(
    pathToFileURL(join(root, "tools", "routing", "routing-render.mjs")).href
  );
  const rendered = renderRoutingTables(table);
  const markers = [
    ["ROUTING-PRIORITY", rendered.priority],
    ["ROUTING-TRIGGER", rendered.trigger],
  ];
  for (const [name, expected] of markers) {
    const re = new RegExp(`<!-- ${name}:START[\\s\\S]*?<!-- ${name}:END -->`);
    const match = skillText.match(re);
    const expectedBlock = `<!-- ${name}:START (generated by tools/routing/render-routing.mjs from tools/routing/routing-table.json — do not edit by hand) -->\n${expected}\n<!-- ${name}:END -->`;
    if (!match || match[0] !== expectedBlock) {
      findings.push(
        makeFinding(
          "routing-table-drift",
          "skills/using-pf/SKILL.md",
          "FAIL",
          "Run node tools/routing/render-routing.mjs to re-render the routing tables from tools/routing/routing-table.json.",
          `The ${name} table drifted from the single source of truth (tools/routing/routing-table.json).`,
        ),
      );
    }
  }
...
export async function runChecks(root, { layers = ["structure", "harness", "orchestration"], coverage } = {}) {
  ...
  if (layers.includes("structure")) {
    await structureChecks(root, findings);
    await routingDriftCheck(root, findings);
  }
