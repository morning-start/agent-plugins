...
```bash
node tools/design/check-tool-names.mjs [--root <plugin-dir>] [--format table|json]
```

`checkToolNames()` (exported from `tools/design/check-tool-names.mjs`) scans every skill
...
- Hooks: which lifecycle events, per `references/harnesses/<harness>/hooks.md` (event names and wiring differ
...
```bash
node tools/design/check-conflicts.mjs --manifest <manifest.json>   # or --root <plugin-dir>
```
...
```bash
node tools/verify/validate-schema.mjs --schema tools/shared/schemas/component-manifest.schema.json --input <manifest.json>
```
...
- Manifest lists per-harness manifest specs (see `references/harnesses/<harness>/plugin.md`).
