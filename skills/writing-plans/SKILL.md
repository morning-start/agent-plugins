...
#### 任务批次（Batch）约束 — 每批最多 5 个任务

多任务执行会拉长上下文窗口（每个任务的测试输出、诊断日志、diff 都会累积）。为控制上下文长度，任务按 **批次（Batch）** 组织：

- **每批最多 5 个任务**（硬上限），批间以「提交 + 会话压缩/新上下文」为检查点
- 批次内任务仍需按依赖拓扑排序；**不要因为凑批而跨 Phase**（Phase 边界优先于批次边界）
- 一个 Phase 的任务超过 5 个 → 拆成多个批次，每批结束都停在提交检查点
- 规划文档中标注批次边界，让执行者明确「完成第 N 批后停止、提交、压缩会话再继续」：

```markdown
## Phase 1: lexer（词法分析模块）— Batch 1: Task 1.1-1.4
## Phase 1: lexer（词法分析模块）— Batch 2: Task 1.5-1.7（批间提交 + 压缩会话）
```

- 若执行平台不支持会话压缩 → 批次上限更严格（建议 ≤3），或逐任务提交后由平台自动汇总

#### 批次划分规则 — 内聚优先 + 执行可行性

批次不是简单的「按序号每 5 个一组」，划分时同时考虑**内聚性**与**执行可行性**：

**① 内聚优先（相同背景的任务尽量放一起）**
- **同模块**：同一 Phase/模块的任务优先同批（共享上下文：类型定义、测试文件、错误处理约定）
- **同文件**：修改同一文件的任务同批（减少文件切换与冲突）
- **同主题**：同一功能点/同一验收目标的子任务同批（测试名、验收项连贯）
- **同依赖链**：互为上下游的任务同批（下游任务可直接复用上游产物，无需跨批回忆）

**② 执行可行性（不只为内聚牺牲执行）**
- **依赖顺序**：批次内任务必须按依赖拓扑排序；有依赖的任务不能因内聚而颠倒顺序
- **可验证性**：每批结束时应有独立验证点（该批涉及的测试全绿 + 可提交）
- **提交粒度**：每批应构成一个合理的提交单元（一个 Conventional Commit 能概括的主题范围）
- **风险隔离**：高风险/探索性任务单独成批，不混入稳定任务批（失败不拖累整批）

**决策顺序**：先按 Phase 分组 → 再按内聚（同模块/同文件/同主题）聚拢 → 最后按执行可行性（依赖/验证/提交）调整，且每批不超过 5 个。

```markdown
# 反例（只按序号分组，内聚差）：
## Batch 1: Task 1.1, 1.2, 2.1, 3.1, 1.3   ← 跨模块混批，上下文频繁切换

# 正例（内聚 + 执行）：
## Phase 1: lexer — Batch 1: Task 1.1, 1.2, 1.3（同模块同文件，依赖顺序）
## Phase 2: parser — Batch 2: Task 2.1, 2.2, 2.3（依赖 Phase 1 产物）
## Phase 3: validate — Batch 3: Task 3.1（风险项单独成批）
```
